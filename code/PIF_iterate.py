...
def run_a_trail(train_config,log_file=None, save_mode=False,save_dir=None,evaluate_only=False,warm_or_cold=None,
            ...
            if config.influence_save_name:
                ...
                for iter, influence_iter in enumerate(influence_iters):
                    if iter == 0:
                        ...
                        if (config.data_name == 'ml-32m') or (config.data_name == 'amazon_book'):
    ...
    for epoch in range(train_config['epoch']):
        ...
        if epoch% train_config['eval_epoch']==0:
            ...
            try:
                train.report(report_dict) # tune.report(**report_dict, training_iteraction = epoch) # train.report(report_dict) # tune.report(**report_dict, training_iteraction = epoch)
            except:
...
if __name__=='__main__':
    ...
    if not evaluate_only:
        ...
        if global_save:
            global_model_path = os.path.join(global_save_dir, 'best_model.pt')
            global_config_path = os.path.join(global_save_dir, 'best_config.pt')
            global_result_path = os.path.join(global_save_dir, 'best_result.pt')
            trail_best_model = torch.load(os.path.join(save_dir,'best_model.pt'))
            trail_best_config = torch.load(os.path.join(save_dir,'best_config.pt'))
            if os.path.exists(global_result_path):
                global_best_result = torch.load(global_result_path)
                trial_best_result = torch.load(os.path.join(global_result_path, 'best_result.pt'))
                if trial_best_result > global_best_result:
                    torch.save(trail_best_model, global_model_path)
                    torch.save(trail_best_config , global_config_path)
            else:
                torch.save(trail_best_model, global_model_path)
                torch.save(trail_best_config , global_config_path)
